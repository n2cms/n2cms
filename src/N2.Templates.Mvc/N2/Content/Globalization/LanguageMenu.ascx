...
<asp:PlaceHolder ID="plhNew" runat="server" Visible='<%# CreatingNew %>'>
	<asp:HyperLink Enabled="false" ID="hlNew" runat="server" CssClass="command" NavigateUrl='<%# "Default.aspx?selected=" + Server.UrlEncode(SelectedItem.Path) %>' ToolTip="<%# CurrentLanguage.LanguageCode %>">
...
<asp:PlaceHolder runat="server" Visible='<%# !CreatingNew %>'>
	<n2:OptionsMenu id="om" runat="server">
		<asp:HyperLink runat="server" CssClass="command plain" NavigateUrl='<%# "Default.aspx?selected=" + Server.UrlEncode(SelectedItem.Path) %>' ToolTip="<%# CurrentLanguage.LanguageCode %>">
