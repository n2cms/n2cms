...
namespace N2.Tests
{
	...
	public static class TestSupport
	{
		...
		public static void Setup(out IDefinitionManager definitions, out ContentActivator activator, out IItemNotifier notifier, out FakeSessionProvider sessionProvider, out ItemFinder finder, out SchemaExport schemaCreator, out InterceptingProxyFactory proxyFactory, out IEventsManager eventsManager, params Type[] itemTypes)
		{
			...
			Setup(out definitions, out activator, out notifier, out sessionProvider, out finder, out schemaCreator, out proxyFactory, context, config, participators, out eventsManager, itemTypes);
		}

		public static void Setup(out IDefinitionManager definitions, out ContentActivator activator, out IItemNotifier notifier, out FakeSessionProvider sessionProvider, out ItemFinder finder, out SchemaExport schemaCreator, out InterceptingProxyFactory proxyFactory, IWebContext context, DatabaseSection config, ConfigurationBuilderParticipator[] participators, out IEventsManager eventsManager, params Type[] itemTypes)
		{
			IDefinitionProvider[] definitionProviders;
			Setup(out definitionProviders, out definitions, out activator, out notifier, out proxyFactory, out eventsManager, itemTypes);
		...
		public static IDefinitionManager SetupDefinitions(out ContentActivator activator, out InterceptingProxyFactory proxyFactory, out IEventsManager eventsManager, params Type[] itemTypes)
		{
			IItemNotifier notifier;
			IDefinitionProvider[] definitionProviders;
			IDefinitionManager definitions;
		...
		public static void Setup(out IDefinitionProvider[] definitionProviders, out IDefinitionManager definitions, out ContentActivator activator, out IItemNotifier notifier, out InterceptingProxyFactory proxyFactory, out IEventsManager eventsManager, params Type[] itemTypes)
		{
			var map = new DefinitionMap();
			...
			eventsManager = SetupEventsManager();
		...
		public static N2.Edit.Versioning.DraftRepository CreateDraftRepository(ref IPersister persister, params Type[] definedTypes)
		{
			...
			IEventsManager events = null;
			return new DraftRepository(CreateVersionRepository(ref persister, ref activator, out events, definedTypes), new CacheWrapper(persister, new ThreadContext(), new DatabaseSection()));
		}
		...
		public static N2.Edit.Versioning.ContentVersionRepository CreateVersionRepository(params Type[] definedTypes)
		{
			...
			ContentActivator activator = null;
			IEventsManager events = null;
			return CreateVersionRepository(ref persister, ref activator, out events, definedTypes);
		}

		public static N2.Edit.Versioning.ContentVersionRepository CreateVersionRepository(ref IPersister persister, ref ContentActivator activator, out IEventsManager events, params Type[] definedTypes)
		{
			IRepository<ContentVersion> versionRepository = null;
			return CreateVersionRepository(ref persister, ref activator, ref versionRepository, out events, definedTypes);
		}

		public static ContentVersionRepository CreateVersionRepository(ref IPersister persister, ref ContentActivator activator, ref IRepository<ContentVersion> versionRepository, out IEventsManager events, params Type[] definedTypes)
		{
			InterceptingProxyFactory proxyFactory;
			var definitions = SetupDefinitions(out activator, out proxyFactory, out events, definedTypes);
			if (persister == null)
