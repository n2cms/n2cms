...
namespace N2.Tests.Definitions
{
	...
	public class DefinitionTests : TypeFindingBase
	{
		...
		{
			var enforcer = new SecurityEnforcer(persister, new SecurityManager(new ThreadContext(), new EditSection()), activator, MockRepository.GenerateStub<IUrlParser>(), new ThreadContext(), new HostSection(), events);
			enforcer.Start();
