...
namespace N2.Management.Tests.ExternalContent
{
	...
	public class ExternalContentRepositoryTests
	{
		...
		public static Externals.ExternalContentRepository SetupRepository(out ContentItem root, out ContentItem start)
		{
			...
			return new Externals.ExternalContentRepository(new Edit.ContainerRepository<Externals.ExternalItem>(persister, finder, new Host(new ThreadContext(), 1, 2), activator) { Navigate = true }, persister, activator, new Configuration.EditSection());
		}
