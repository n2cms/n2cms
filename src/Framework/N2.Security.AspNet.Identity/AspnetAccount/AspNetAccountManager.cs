...
namespace N2.Security.AspNet.Identity
{
    ...
    /// <seealso cref="RoleStore"/>
    /// <seealso cref="AspNetAccountResources"/>
    /// </summary>
    /// <remarks>
    ...
    /// </ul>  
    /// </remarks>
    public abstract class AspNetAccountManager<TUser> : AccountManager where TUser : ContentUser
	{
        public AspNetAccountManager(UserStore<TUser> userStore, RoleStore roleStore)
		{
            UserStore = userStore; UserManager = userStore.GetUserManager(); 
            RoleStore = roleStore; RoleManager = roleStore.GetRoleManager();
		} 

        protected UserStore<TUser>   UserStore   { get; private set; }
        ...
        public override bool ChangePassword(string userName, string newPassword)
        {
            ...
            if (user == null)
                return false;
            UserManager.AddPassword(user.UserId, newPassword); // will store hash of the password // todo: NHibernate async
            return true;
        }
