	...
	public partial class Diagnose : Page
	{
		protected override void OnPreRender(EventArgs e)
		{
			...
			try
			{
				...
				lblEditVersion.Text = (new AssemblyName(Assembly.Load("N2.Edit").FullName)).Version.ToString();
			}
			catch (Exception ex)
			{
		...
		{
			get { return new InstallationManager(N2.Context.Instance); }
		}
		...
		protected void btnInsertRootNode_Click(object sender, EventArgs e)
		{
			try
			{
				...
				if (itemID == N2.Context.Instance.Resolve<Site>().RootItemID)
					lblInsert.Text =
