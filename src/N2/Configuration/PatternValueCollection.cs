...
namespace N2.Configuration
{
	...
	public class PatternValueCollection : ConfigurationElementCollection
	{
		...
		{
			BaseAdd(new PatterValueElement("smallA", "[Â‰·‡‚„@]", "a", true));
			BaseAdd(new PatterValueElement("capitalA", "[≈ƒ¡¿¬√]", "a", true));
			BaseAdd(new PatterValueElement("smallAE", "[Ê]", "ae", true));
			BaseAdd(new PatterValueElement("capitalAE", "[∆]", "AE", true));
			BaseAdd(new PatterValueElement("smallO", "[ˆ¯Ùı]", "o", true));
			BaseAdd(new PatterValueElement("capitalO", "[÷ÿ‘’]", "O", true));
